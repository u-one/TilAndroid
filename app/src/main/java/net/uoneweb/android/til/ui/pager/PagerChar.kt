...
sealed class PagerChar(val code: PagerCode, val size: IntSize, charData: String) {
    val charData: IntArray =
        charData.trim().split("\n").flatMap { line ->
            line.trim().map { if (it == '1') 1 else 0 }
        }.toIntArray()

    class Alpha(code: PagerCode, val char: String, charData: String) :
        PagerChar(code, IntSize(5, 7), charData) {
    }
